<template>
  ...
  >
    ...
    <VcTable
      ...
      :search-placeholder="$t('ONE_SHELL_APP.PAGES.LIST.SEARCH.PLACEHOLDER')"
      :total-label="$t('ONE_SHELL_APP.PAGES.LIST.TABLE.TOTALS')"
      state-key="ONE_SHELL_APP"
      :items="data"
...
import { useOneShellAppList } from "./../composables";
...
defineOptions({
  url: "/one-shell-app",
  name: "OneShellAppList",
  isWorkspace: true,
  menuItem: {
    title: "ONE_SHELL_APP.MENU.TITLE",
    icon: "lucide-file",
    priority: 1,
  },
});
...
const { getItems, data, loading, totalCount, pages, currentPage, searchQuery, removeItems } = useOneShellAppList({
...
const empty = {
  icon: "lucide-file",
  text: computed(() => t("ONE_SHELL_APP.PAGES.LIST.EMPTY.NO_ITEMS")),
  action: computed(() => t("ONE_SHELL_APP.PAGES.LIST.EMPTY.ADD")),
  clickHandler: () => {
    addItem();
  },
};
...
const notfound = {
  icon: "lucide-file",
  text: computed(() => t("ONE_SHELL_APP.PAGES.LIST.NOT_FOUND.EMPTY")),
  action: computed(() => t("ONE_SHELL_APP.PAGES.LIST.NOT_FOUND.RESET")),
  clickHandler: async () => {
    searchValue.value = "";
    await getItems({
      ...searchQuery.value,
...
const bladeToolbar = ref<IBladeToolbar[]>([
  {
    id: "refresh",
    title: computed(() => t("ONE_SHELL_APP.PAGES.LIST.TOOLBAR.REFRESH")),
  ...
  },
  {
    id: "add",
    icon: "material-add",
    title: computed(() => t("ONE_SHELL_APP.PAGES.LIST.TOOLBAR.ADD")),
    clickHandler: () => {
      addItem();
    },
  },
...
const title = computed(() => t("ONE_SHELL_APP.PAGES.LIST.TITLE"));
...
async function remove(ids: string[]) {
  if (
    await showConfirmation(
      t(`ONE_SHELL_APP.PAGES.ALERTS.DELETE_SELECTED_CONFIRMATION.MESSAGE`, {
